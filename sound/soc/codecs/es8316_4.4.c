...
/** **/
static int snd_soc_read(struct snd_soc_component *component, unsigned int reg){
...
static int snd_soc_update_bits(struct snd_soc_component *component, unsigned int reg, unsigned int mask, unsigned int val){
 ...
 */
static int es8316_reset(struct snd_soc_component *codec)
{
			      ...
			      struct snd_soc_dai *dai)
{
	...
	if (playback) {
		...
		snd_soc_update_bits(codec, ES8316_GPIO_SEL_REG4D,0x02,0x2);
...
//    struct snd_soc_codec *codec = rtd->codec;
	...
	if (playback) {
		...
		snd_soc_update_bits(codec, ES8316_GPIO_SEL_REG4D,0x02,0x0);
...
static int es8316_set_bias_level(struct snd_soc_component *codec,
...
static int es8316_init_regs(struct snd_soc_component *codec)
{
	snd_soc_write(codec, ES8316_RESET_REG00, 0x3f);
	usleep_range(5000, 5500);
	snd_soc_write(codec, ES8316_RESET_REG00, 0x00);
...
static int es8316_suspend(struct snd_soc_component *codec)
{
	return 0;
}
...
int es8316_headset_detect(int jack_insert)
{
	...
	es8316 = snd_soc_component_get_drvdata(es8316_codec);
...
static int es8316_probe(struct snd_soc_component *codec)
{
	struct es8316_priv *es8316 = snd_soc_component_get_drvdata(codec);
	int ret = 0;
	es8316_codec = codec;
...
static struct snd_soc_component_driver soc_codec_dev_es8316 = {
	.probe =	es8316_probe,
