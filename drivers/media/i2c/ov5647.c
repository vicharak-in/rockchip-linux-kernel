...
static int ov5647_reg_verify(struct i2c_client *client,
				const struct regval *regs)
{
	u32 i;
	int ret = 0;
	...
	for (i = 0; ret == 0 && regs[i].addr != REG_NULL; i++) {
		ret = ov5647_read_reg(client, regs[i].addr,
			  OV5647_REG_VALUE_08BIT, &value);
		if (value != regs[i].val) {
...
static int __ov5647_start_stream(struct ov5647 *ov5647)
{
	...
	dev_info(&ov5647->client->dev, "%s:Check register value!\n",
				__func__);
	ret = ov5647_reg_verify(ov5647->client, ov5647_global_regs);
	if (ret)
		return ret;

	ret = ov5647_reg_verify(ov5647->client, ov5647->cur_mode->reg_list);
	if (ret)
		return ret;
#endif
...
static int __ov5647_power_on(struct ov5647 *ov5647)
{
	...
	if (!IS_ERR(ov5647->power_gpio))
		gpiod_set_value_cansleep(ov5647->power_gpio, 1);
	...
	ret = clk_set_rate(ov5647->xvclk, OV5647_XVCLK_FREQ);
	if (ret < 0)
		dev_warn(dev, "Failed to set xvclk rate (24MHz)\n");
	if (clk_get_rate(ov5647->xvclk) != OV5647_XVCLK_FREQ)
	...
	delay_us = ov5647_cal_delay(8192);
			...
			const struct i2c_device_id *id)
{
	...
	ov5647->xvclk = devm_clk_get(dev, "xvclk");
	if (IS_ERR(ov5647->xvclk)) {
