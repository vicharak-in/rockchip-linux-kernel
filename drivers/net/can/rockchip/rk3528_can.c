...
#define DRV_NAME	"rk3528_can"

/* rk3528_can private data structure */
...
static inline u32 rk3528_can_read(const struct rk3528_can *priv,
				  enum rk3528_can_reg reg)
{
	return readl(priv->base + reg);
}

static inline void rk3528_can_write(const struct rk3528_can *priv,
				    enum rk3528_can_reg reg, u32 val)
{
	writel(val, priv->base + reg);
}

static const struct can_bittiming_const rk3528_can_bittiming_const = {
	.name = DRV_NAME,
	.tseg1_min = 1,
	.tseg1_max = 128,
	.tseg2_min = 1,
...
static const struct can_bittiming_const rk3528_can_data_bittiming_const = {
	.name = DRV_NAME,
	.tseg1_min = 1,
	.tseg1_max = 32,
	.tseg2_min = 1,
...
static int set_reset_mode(struct net_device *ndev)
{
	...
	rk3528_can_write(rcan, CAN_MODE, 0);

	netdev_dbg(ndev, "%s MODE=0x%08x\n", __func__,
		   rk3528_can_read(rcan, CAN_MODE));
...
static int set_normal_mode(struct net_device *ndev)
{
	...
	val = rk3528_can_read(rcan, CAN_MODE);
	val |= WORK_MODE;
	rk3528_can_write(rcan, CAN_MODE, val);
...
static int rk3528_can_set_bittiming(struct net_device *ndev)
{
	struct rk3528_can *rcan = netdev_priv(ndev);
	...
	rk3528_can_write(rcan, CAN_NBTP, reg_btp);
	...
	if (rcan->can.ctrlmode & CAN_CTRLMODE_FD) {
		...
		if (dbt->bitrate > 2200000) {
			...
			rk3528_can_write(rcan, CAN_TDCR,
		...
		rk3528_can_write(rcan, CAN_DBTP, reg_btp);
	}

	netdev_dbg(ndev, "%s NBTP=0x%08x, DBTP=0x%08x, TDCR=0x%08x\n", __func__,
		   rk3528_can_read(rcan, CAN_NBTP),
		   rk3528_can_read(rcan, CAN_DBTP),
		   rk3528_can_read(rcan, CAN_TDCR));
	return 0;
}

static int rk3528_can_get_berr_counter(const struct net_device *ndev,
				       struct can_berr_counter *bec)
{
	...
	bec->rxerr = rk3528_can_read(rcan, CAN_RX_ERR_CNT);
	bec->txerr = rk3528_can_read(rcan, CAN_TX_ERR_CNT);

	pm_runtime_put(rcan->dev);

	netdev_dbg(ndev, "%s RX_ERR_CNT=0x%08x, TX_ERR_CNT=0x%08x\n", __func__,
		   rk3528_can_read(rcan, CAN_RX_ERR_CNT),
		   rk3528_can_read(rcan, CAN_TX_ERR_CNT));

	return 0;
}

static int rk3528_can_start(struct net_device *ndev)
{
	struct rk3528_can *rcan = netdev_priv(ndev);
	u32 val;
	...
	rk3528_can_write(rcan, CAN_IDMASK, CAN_RX_FILTER_MASK);
	rk3528_can_write(rcan, CAN_IDCODE0, 0);
	rk3528_can_write(rcan, CAN_IDMASK0, CAN_RX_FILTER_MASK);
	rk3528_can_write(rcan, CAN_IDCODE1, 0);
	rk3528_can_write(rcan, CAN_IDMASK1, CAN_RX_FILTER_MASK);
	rk3528_can_write(rcan, CAN_IDCODE2, 0);
	rk3528_can_write(rcan, CAN_IDMASK2, CAN_RX_FILTER_MASK);
	rk3528_can_write(rcan, CAN_IDCODE3, 0);
	rk3528_can_write(rcan, CAN_IDMASK3, CAN_RX_FILTER_MASK);
	rk3528_can_write(rcan, CAN_IDCODE4, 0);
	rk3528_can_write(rcan, CAN_IDMASK4, CAN_RX_FILTER_MASK);

	/* set mode */
	val = rk3528_can_read(rcan, CAN_MODE);

	/* rx fifo enable */
	rk3528_can_write(rcan, CAN_RXFC,
			 rk3528_can_read(rcan, CAN_RXFC) | FIFO_ENABLE |
	...
	netdev_dbg(ndev, "%s MODE=0x%08x, INT_MASK=0x%08x\n", __func__,
		   rk3528_can_read(rcan, CAN_MODE),
		   rk3528_can_read(rcan, CAN_INT_MASK));

	return 0;
}

static int rk3528_can_stop(struct net_device *ndev)
{
	struct rk3528_can *rcan = netdev_priv(ndev);
	...
	rk3528_can_write(rcan, CAN_INT_MASK, 0xffff);
...
static int rk3528_can_set_mode(struct net_device *ndev,
			       enum can_mode mode)
{
	int err;
 ...
 */
static int rk3528_can_start_xmit(struct sk_buff *skb,
				 struct net_device *ndev)
{
	...
	if (rk3528_can_read(rcan, CAN_CMD) & CAN_TX0_REQ)
		cmd = CAN_TX1_REQ;
	...
	rk3528_can_write(rcan, CAN_TXID, id);
	rk3528_can_write(rcan, CAN_TXFIC, dlc);

	for (i = 0; i < cf->len; i += 4)
		rk3528_can_write(rcan, CAN_TXDAT0 + i,
				 *(u32 *)(cf->data + i));

	can_put_echo_skb(skb, ndev, 0);

	rk3528_can_write(rcan, CAN_CMD, cmd);

	return NETDEV_TX_OK;
}

static int rk3528_can_rx(struct net_device *ndev)
{
	struct rk3528_can *rcan = netdev_priv(ndev);
	struct net_device_stats *stats = &ndev->stats;
	struct canfd_frame *cf;
	struct sk_buff *skb;
	u32 id_rk3528_can, dlc;
	int i = 0;
	u32 __maybe_unused ts, ret;
	u32 data[16] = {0};

	dlc = rk3528_can_read(rcan, CAN_RXFRD);
	id_rk3528_can = rk3528_can_read(rcan, CAN_RXFRD);
	for (i = 0; i < (rcan->rx_max_data - 2); i++)
		data[i] = rk3528_can_read(rcan, CAN_RXFRD);

	if (rcan->mode == ROCKCHIP_RK3528_CAN_MODE) {
		/* may be an empty frame */
		if (!dlc && !id_rk3528_can)
			return 1;
	}
...
/* rk3528_can_rx_poll - Poll routine for rx packets (NAPI)
 ...
 */
static int rk3528_can_rx_poll(struct napi_struct *napi, int quota)
{
...
static int rk3528_can_err(struct net_device *ndev, u32 isr)
{
	struct rk3528_can *rcan = netdev_priv(ndev);
	struct net_device_stats *stats = &ndev->stats;
	struct can_frame *cf;
	...
	rxerr = rk3528_can_read(rcan, CAN_RX_ERR_CNT);
	txerr = rk3528_can_read(rcan, CAN_TX_ERR_CNT);
	sta_reg = rk3528_can_read(rcan, CAN_STATE);
...
static irqreturn_t rk3528_can_interrupt(int irq, void *dev_id)
{
	...
	if (isr & TX_FINISH_INT) {
		dlc = rk3528_can_read(rcan, CAN_TXFIC);
		...
		stats->tx_packets++;
		rk3528_can_write(rcan, CAN_CMD, 0);
	...
	if (isr & err_int) {
		/* error interrupt */
		if (rk3528_can_err(ndev, isr))
			netdev_err(ndev, "can't allocate buffer - clearing pending interrupts\n");
	}

	rk3528_can_write(rcan, CAN_INT, isr);
	return IRQ_HANDLED;
}

static int rk3528_can_open(struct net_device *ndev)
{
	struct rk3528_can *rcan = netdev_priv(ndev);
	int err;
...
static int rk3528_can_close(struct net_device *ndev)
{
	struct rk3528_can *rcan = netdev_priv(ndev);

	netif_stop_queue(ndev);
	napi_disable(&rcan->napi);
	rk3528_can_stop(ndev);
	close_candev(ndev);
...
static const struct net_device_ops rk3528_can_netdev_ops = {
	.ndo_open = rk3528_can_open,
	.ndo_stop = rk3528_can_close,
	.ndo_start_xmit = rk3528_can_start_xmit,
	.ndo_change_mtu = can_change_mtu,
};

/**
 * rk3528_can_suspend - Suspend method for the driver
 * @dev:	Address of the device structure
 *
 * Put the driver into low power mode.
 * Return: 0 on success and failure value on error
 */
static int __maybe_unused rk3528_can_suspend(struct device *dev)
{
...
/**
 * rk3528_can_resume - Resume from suspend
 * @dev:	Address of the device structure
 *
 * Resume operation after suspend.
 * Return: 0 on success and failure value on error
 */
static int __maybe_unused rk3528_can_resume(struct device *dev)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	int ret;
	...
	if (netif_running(ndev)) {
		ret = rk3528_can_start(ndev);
		if (ret) {
			dev_err(dev, "rk3528_can_chip_start failed on resume\n");
			return ret;
		}
...
/**
 * rk3528_can_runtime_suspend - Runtime suspend method for the driver
 * @dev:	Address of the device structure
 *
 * Put the driver into low power mode.
 * Return: 0 always
 */
static int __maybe_unused rk3528_can_runtime_suspend(struct device *dev)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	struct rk3528_can *rcan = netdev_priv(ndev);
...
/**
 * rk3528_can_runtime_resume - Runtime resume from suspend
 * @dev:	Address of the device structure
 *
 * Resume operation after suspend.
 * Return: 0 on success and failure value on error
 */
static int __maybe_unused rk3528_can_runtime_resume(struct device *dev)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	struct rk3528_can *rcan = netdev_priv(ndev);
	int ret;
...
static const struct dev_pm_ops rk3528_can_dev_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(rk3528_can_suspend, rk3528_can_resume)
	SET_RUNTIME_PM_OPS(rk3528_can_runtime_suspend,
			   rk3528_can_runtime_resume, NULL)
};

static const struct of_device_id rk3528_can_of_match[] = {
	{
		.compatible = "rockchip,rk3528-can",
		.data = (void *)ROCKCHIP_RK3528_CAN_MODE
	},
	{},
};
MODULE_DEVICE_TABLE(of, rk3528_can_of_match);

static int rk3528_can_probe(struct platform_device *pdev)
{
	struct net_device *ndev;
	struct rk3528_can *rcan;
	struct resource *res;
	void __iomem *addr;
	int err, irq;
	...
	ndev = alloc_candev(sizeof(struct rk3528_can), 1);
	if (!ndev) {
	...
	err = devm_request_irq(&pdev->dev, irq, rk3528_can_interrupt,
			       0, ndev->name, ndev);
	if (err) {
	...
	rcan->can.state = CAN_STATE_STOPPED;
	switch (rcan->mode) {
	case ROCKCHIP_RK3528_CAN_MODE:
		rcan->can.bittiming_const = &rk3528_can_bittiming_const;
		rcan->can.do_set_mode = rk3528_can_set_mode;
		rcan->can.do_get_berr_counter = rk3528_can_get_berr_counter;
	...
	ndev->netdev_ops = &rk3528_can_netdev_ops;
	ndev->irq = irq;
	ndev->flags |= IFF_ECHO;
	...
	netif_napi_add(ndev, &rcan->napi, rk3528_can_rx_poll,
		       rcan->rx_fifo_depth);
...
static int rk3528_can_remove(struct platform_device *pdev)
{
...
static struct platform_driver rk3528_can_driver = {
	.driver = {
		.name = DRV_NAME,
		.pm = &rk3528_can_dev_pm_ops,
		.of_match_table = rk3528_can_of_match,
	},
	.probe = rk3528_can_probe,
	.remove = rk3528_can_remove,
};
module_platform_driver(rk3528_can_driver);

MODULE_AUTHOR("Elaine Zhang <zhangqing@rock-chips.com>");
MODULE_DESCRIPTION("RK3528 CAN Drivers");
