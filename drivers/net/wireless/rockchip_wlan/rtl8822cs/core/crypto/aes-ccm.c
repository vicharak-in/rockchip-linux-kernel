			       ...
			       u8 *x)
{
	...
	aes_rtl8822cs_encrypt(aes, b, x); /* X_1 = E(K, B_0) */
	...
	aes_rtl8822cs_encrypt(aes, aad_buf, x); /* X_2 = E(K, X_1 XOR B_1) */

	if (aad_len > AES_BLOCK_SIZE - 2) {
		xor_aes_block(&aad_buf[AES_BLOCK_SIZE], x);
		/* X_3 = E(K, X_2 XOR B_2) */
		aes_rtl8822cs_encrypt(aes, &aad_buf[AES_BLOCK_SIZE], x);
	}
}
			 ...
			 u8 *a)
{
	...
	for (i = 1; i <= len / AES_BLOCK_SIZE; i++) {
		...
		/* S_i = E(K, A_i) */
		aes_rtl8822cs_encrypt(aes, a, out);
...
static void aes_ccm_encr_auth(void *aes, size_t M, u8 *x, u8 *a, u8 *auth)
{
	...
	aes_rtl8822cs_encrypt(aes, a, tmp);
	for (i = 0; i < M; i++)
		auth[i] = x[i] ^ tmp[i];
